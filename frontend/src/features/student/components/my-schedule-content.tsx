  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!classCode || !yearCode) {
          if (mounted) {
            setEntries([]);
            setStudentClassCode(classCode);
            setStudentClassName(className);
            setAcademicYearCode(yearCode);
            setError("Chưa xác định được lớp hoặc năm học hiện tại của học sinh.");
          }
          return;
        }

        const timetableResponse = await teachingApi.getTimetableGrid(semesterCode, yearCode);
        const allEntries = Array.isArray(timetableResponse.data) ? timetableResponse.data : [];
        ...
        setAcademicYearCode(yearCode);
        setEntries(allEntries.filter((item) => normalizeClassCode(item.classCode) === classCode));
      } catch (loadError) {
        if (!mounted) {
          return;
        }
  ...
  const groupedByDay = useMemo(() => {
    return DAYS.map((day) => {
      const lessons = entries
        ...
        .sort((left, right) => Number(left.periodStart) - Number(right.periodStart))
        .map((item) => ({
    ...
    <div className="grid gap-4">
      ...
      <Panel className="p-4">
        <div className="grid gap-3 sm:grid-cols-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Lớp</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">{studentClassName || studentClassCode || "--"}</p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Năm học</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">{academicYearCode || "--"}</p>
          </div>
          <div className="grid gap-1">
            <label htmlFor="my-schedule-semester" className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
              Học kỳ
            </label>
            <select
              id="my-schedule-semester"
              value={semesterCode}
              onChange={(event) => setSemesterCode(event.target.value as SemesterCode)}
      ...
      {error ? (
        <Panel className="border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">{error}</Panel>
      ) : null}

      {isLoading ? (
        <Panel className="p-4 text-sm text-slate-500">Đang tải thời khóa biểu...</Panel>
      ) : null}

      {!isLoading && !error && groupedByDay.length === 0 ? (
        <Panel className="p-4 text-sm text-slate-500">Chưa có dữ liệu thời khóa biểu cho lớp hiện tại trong học kỳ đã chọn.</Panel>
      ) : null}

      {groupedByDay.map((daySchedule) => (
        <div key={daySchedule.day} className="grid gap-4">
          <h3 className="text-lg font-semibold text-slate-900">{daySchedule.day}</h3>
          <Panel className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="px-4 py-3 text-left font-semibold text-slate-900">Giờ học</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-900">Môn học</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-900">Phòng</th>
                    <th className="px-4 py-3 text-left font-semibold text-slate-900">Giáo viên</th>
                  </tr>
                </thead>
                <tbody>
                  {daySchedule.lessons.map((lesson) => (
                    ...
                    >
                      <td className="px-4 py-3 font-medium text-slate-900">{lesson.time}</td>
                      <td className="px-4 py-3 text-slate-900">{lesson.subject}</td>
                      <td className="px-4 py-3">
                        <span className="inline-block rounded-lg bg-blue-100 px-2 py-1 text-xs font-medium text-blue-900">
                          {lesson.room}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-600">{lesson.teacher}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
        ...
        <ul className="mt-2 list-inside list-disc space-y-1">
          ...
          <li>Dữ liệu hiện lấy theo tiết học và môn học từ bảng thời khóa biểu chính thức.</li>
        </ul>
      </div>
    </div>
  );
